...
function seedDb() {
  ...
  return {
    ...
    projects: [{
      ...
      assignments: [
        { id: 'assign_maya', userId: 'user_maya', agent: 'maya-agent', status: 'returned', task: 'Hero film opening and edit rhythm', pushedAt: created, returnedAt: '2026-08-18T12:20:00.000Z' },
        { id: 'assign_jonas', userId: 'user_jonas', agent: 'jonas-agent', status: 'working', task: 'Human performance beat before reveal', pushedAt: created, returnedAt: null },
        { id: 'assign_asha', userId: 'user_asha', agent: 'asha-agent', status: 'returned', task: 'Product macro stills and mark reveal', pushedAt: created, returnedAt: '2026-08-18T13:05:00.000Z' },
        { id: 'assign_leo', userId: 'user_leo', agent: 'leo-agent', status: 'delivered', task: 'Packaging stills and handoff notes', pushedAt: created, returnedAt: null }
      ],
      createdAt: created
    }],
    assets,
...
async function ensureStorage(dataDir) {
  ...
  try {
    ...
    if (terminologyChanged) await saveDb(dataDir, parsed);
    return parsed;
  } catch (error) {
...
class Repository {
  ...
  pluginPackets(token) {
    ...
    const projects = isLeader ? db.projects : db.projects.filter(project =>
      (project.assignments || []).some(assignment => assignment.userId === user.id || assignment.agent === user.agent)
    );
    return {
      ...
      packets: projects.map(project => {
        const assignments = (project.assignments || []).filter(assignment =>
          isLeader || assignment.userId === user.id || assignment.agent === user.agent
        );
        return {
          project,
          assignments,
  ...
  pushPacket(input, actor) {
    return this.mutate(db => {
      ...
      db.activity.unshift({ id: id('act'), type: 'packet_pushed', message: `${actor} pushed ${project.name} to ${created.length} AI coding agents`, actor, at: now() });
  ...
  updateAssignment(assignmentId, input, actor) {
    return this.mutate(db => {
      for (const project of db.projects) {
        ...
        db.activity.unshift({ id: id('act'), type: 'assignment_status', message: `${actor} marked ${assignment.agent} ${status} on ${project.name}`, actor, at: now() });
