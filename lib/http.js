...
async function serveStatic(request, response, pathname) {
  ...
  let target = path.resolve(PUBLIC_ROOT, relative);
...
async function handleApi(request, response, url, repository) {
  ...
  const requestOrigin = `${request.headers['x-forwarded-proto'] || 'http'}://${request.headers.host || '127.0.0.1'}`;
  const sendBlob = async (hash, mime, filename, cacheControl = 'private, max-age=3600') => {
    const target = path.join(repository.dataDir, 'blobs', hash.slice(0, 2), hash);
    try {
      ...
      error(response, 404, 'File bytes unavailable');
    }
  };
  const sendCreatedFile = async versionId => {
    ...
    return sendBlob(version.hash, version.mime, version.filename, 'private, max-age=31536000, immutable');
  };
  ...
  if (url.pathname === '/api/plugin/mcp-connections') {
    if (request.method === 'POST') {
      const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
      return json(response, 201, await repository.createMcpConnection(authToken, input));
    }
    if (request.method === 'GET') {
      return json(response, 200, { connections: repository.listMcpConnections(authToken) });
    }
  }
  const mcpConnectionMatch = url.pathname.match(/^\/api\/plugin\/mcp-connections\/([A-Za-z0-9_-]+)$/);
  if (mcpConnectionMatch && request.method === 'DELETE') {
    return json(response, 200, { connection: await repository.revokeMcpConnection(authToken, mcpConnectionMatch[1]) });
  }
  const inviteMatch = url.pathname.match(/^\/api\/plugin\/invites\/([A-Za-z0-9_-]+)\/accept$/);
  if (inviteMatch && request.method === 'POST') {
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 200, await repository.acceptInvite(inviteMatch[1], input));
  }
  if (request.method === 'GET' && url.pathname === '/api/plugin/packets') {
    const token = url.searchParams.get('token') || request.headers['x-nitrate-plugin-token'] || authToken;
    return json(response, 200, repository.pluginPackets(token));
  }
  if (request.method === 'POST' && url.pathname === '/api/plugin/push') {
    const found = repository.requirePluginSession(authToken, 'team_lead');
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    const result = await repository.pushPacket(input, found.user.name, found.user.id);
    return json(response, 201, {
      ...result,
      invitations: result.invitations.map(invite => ({
        ...invite,
        inviteUrl: `${requestOrigin}/join/${invite.token}`
      }))
    });
  }
  const packetInputMatch = url.pathname.match(/^\/api\/plugin\/packets\/([^/]+)\/inputs$/);
  if (packetInputMatch && request.method === 'POST') {
    const found = repository.requirePluginSession(authToken, 'team_lead');
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 201, await repository.reservePacketInput(packetInputMatch[1], input, found.user.name, found.user.id));
  }
  const inputRawMatch = url.pathname.match(/^\/api\/plugin\/inputs\/([^/]+)\/raw$/);
  if (inputRawMatch && request.method === 'PUT') {
    const found = repository.requirePluginSession(authToken, 'team_lead');
    const bytes = await readBody(request, MAX_UPLOAD_BYTES + 1);
    return json(response, 200, await repository.uploadReservedPacketInput(inputRawMatch[1], bytes, found.user.id));
  }
  if (inputRawMatch && request.method === 'GET') {
    const item = repository.packetInput(inputRawMatch[1], authToken);
    return sendBlob(item.hash, item.mime, item.filename, 'private, max-age=31536000, immutable');
  }
  const assignmentMatch = url.pathname.match(/^\/api\/plugin\/assignments\/([^/]+)$/);
  if (assignmentMatch && request.method === 'PATCH') {
    const found = repository.requirePluginSession(authToken);
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 200, await repository.updateAssignment(assignmentMatch[1], input, found.user.name, found.user));
  }
  const assignmentReturnMatch = url.pathname.match(/^\/api\/plugin\/assignments\/([^/]+)\/returns$/);
  if (assignmentReturnMatch && request.method === 'POST') {
    const found = repository.requirePluginSession(authToken);
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 201, await repository.reserveReturn(assignmentReturnMatch[1], input, found.user));
  }
  const pluginReturnRawMatch = url.pathname.match(/^\/api\/plugin\/returns\/([^/]+)\/raw$/);
  if (pluginReturnRawMatch && request.method === 'PUT') {
    const found = repository.requirePluginSession(authToken);
    const bytes = await readBody(request, MAX_UPLOAD_BYTES + 1);
    return json(response, 201, await repository.uploadReservedReturn(pluginReturnRawMatch[1], bytes, found.user));
  }
  const pluginReturnMatch = url.pathname.match(/^\/api\/plugin\/returns\/([^/]+)$/);
  if (pluginReturnMatch && request.method === 'PATCH') {
    const found = repository.requirePluginSession(authToken, 'team_lead');
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 200, await repository.reviewReturn(pluginReturnMatch[1], input, found.user));
  }
  if (request.method === 'POST' && url.pathname === '/api/packets') {
    const found = repository.requirePluginSession(authToken, 'team_lead');
    const input = JSON.parse((await readBody(request, 1024 * 1024)).toString('utf8') || '{}');
    return json(response, 201, await repository.createProject(input, found.user.name, found.user.id));
  }
  ...
  if (request.method === 'POST' && url.pathname === '/api/waitlist') {
    ...
    const input = isBrowserForm
      ...
      : JSON.parse(raw || '{}');
    const result = await repository.addToWaitlist(input);
    if (isBrowserForm) {
...
async function handler(request, response, repository) {
  ...
  try {
    if (url.pathname.startsWith('/api/')) {
      ...
      response.setHeader('Access-Control-Allow-Headers', 'Content-Type,Authorization,X-Nitrate-Plugin-Token,X-Nitrate-Bootstrap-Secret,X-Reel-User,X-Reel-Role');
    ...
    if (url.pathname === '/app' || url.pathname === '/app/') return serveStatic(request, response, '/app/index.html');
    ...
    if (url.pathname === '/press' || url.pathname === '/press/') return serveStatic(request, response, '/press/index.html');
...
function createServer(options = {}) {
  ...
  const repositoryPromise = new Repository(dataDir, { seedDemo: options.seedDemo }).init();
